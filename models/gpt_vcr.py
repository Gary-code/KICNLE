...
from transformers import GPT2Tokenizer, BertConfig, BertTokenizerFast
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        assert len(self.tokenizer) == orig_num_tokens + num_new_tokens
        ...
        self.o_segment_id, self.q_segment_id, self.a_segment_id, self.e_segment_id , self.no_obj_segment_id = \
            self.tokenizer.convert_tokens_to_ids(['<objects>', '<question>', '<answer>', '<explanation>', '<noobject>'])

        # seq len
        self.explanation_max_len = 40
        self.answer_max_len = 20
        self.question_max_len = 20
    ...
    def forward(
        self,
        input_ids_init_ans, labels_init_ans, segment_init_ans_ids, boxes_init_ans, box_numbers_init_ans, \
        input_ids_to_ex, labels_to_ex, segment_to_ex_ids, boxes_to_ex, box_numbers_to_ex, \
        input_ids_to_ans, labels_to_ans, segment_to_ans_ids, boxes_to_ans, box_numbers_to_ans,
        past_key_values=None,
        attention_mask=None,
        position_ids=None,
        head_mask=None,
        ...
        transformer_outputs_init_ans = self.transformer(
            ...
            encoder_hidden_states=encoder_hidden_states,
            encoder_attention_mask=encoder_attention_mask,
            boxes = boxes_init_ans,
            box_numbers = box_numbers_init_ans,
            use_cache=use_cache,
            output_attentions=output_attentions,
        ...
        for i in range(self.iteration_num):
            ...
            # encoder_attention_mask=encoder_attention_mask,
            # boxes = boxes_to_ans,
            # box_numbers = box_numbers_to_ans,
            # use_cache=use_cache,
            # output_attentions=output_attentions,
            # output_hidden_states=output_hidden_states,
            # return_dict=return_dict,
        ...
        # loss = 0.7 * loss_init_ans + 0.3 * (loss_ex + loss_ans) / self.iteration_num
        loss = 0.3 * loss_init_ans + 0.7 * loss_ex
        # loss = (loss_ex + loss_ans) / self.iteration_num
    ...
    def _sample(
        ...
        boxes_init_ans = torch.cat((boxes, torch.zeros(answer_init.shape[1],8).unsqueeze(0).to(input_ids.device)), dim=1)
        no_object_pad_init_ans = torch.LongTensor([self.special_tokens_ids[-1]] * answer_init.shape[1]).to(input_ids.device)

        box_numbers_init_ans = torch.cat((box_numbers, no_object_pad_init_ans.unsqueeze(0)), dim = 1)
        for step in range(self.answer_max_len + 1):
            ...
            if prev_ans.item() in self.special_tokens_ids:
                break
            new_segment = torch.LongTensor([self.special_tokens_ids[-3]]).to(input_ids.device).unsqueeze(dim=0)  # answer segment
            ...
            boxes_init_ans = torch.cat((boxes_init_ans, torch.zeros(1,8).unsqueeze(0).to(input_ids.device)), dim = 1)
            no_object = torch.LongTensor([self.special_tokens_ids[-1]]).to(input_ids.device)

            box_numbers_init_ans = torch.cat((box_numbers_init_ans, no_object.unsqueeze(0)), dim = 1)
        ...
        for i in range(self.iteration_num):
            ...
            boxes_to_ex = torch.cat((boxes, torch.zeros(answer_init[:, 1:].shape[1] + answer_iter_ids.shape[1] + len(ex_head_token), 8).unsqueeze(0).to(input_ids.device)), dim=1)
            no_object_pad_to_ex = torch.LongTensor([self.special_tokens_ids[-1]] * (answer_init[:, 1:].shape[1] + answer_iter_ids.shape[1] + len(ex_head_token))).to(input_ids.device)
            box_numbers_to_ex = torch.cat((box_numbers, no_object_pad_to_ex.unsqueeze(0)), dim = 1)
            ...
            for step in range(self.explanation_max_len + 1):
                ...
                if prev_ex.item() in self.special_tokens_ids:
                    break
                new_segment = torch.LongTensor([self.special_tokens_ids[-2]]).to(input_ids.device).unsqueeze(dim=0)  # explanation segment
                ...
                boxes_to_ex = torch.cat((boxes_to_ex, torch.zeros(1,8).unsqueeze(0).to(input_ids.device)), dim = 1)
                no_object = torch.LongTensor([self.special_tokens_ids[-1]]).to(input_ids.device)

                box_numbers_to_ex = torch.cat((box_numbers_to_ex, no_object.unsqueeze(0)), dim = 1)
            ...
            boxes_to_ans = torch.cat((boxes, torch.zeros(explanation_iter[:, 1:].shape[1] + explanation_iter_ids.shape[1] + answer_iter.shape[1], 8).unsqueeze(0).to(input_ids.device)), dim=1)
            no_object_pad_to_ans = torch.LongTensor([self.special_tokens_ids[-1]] * (explanation_iter[:, 1:].shape[1] + explanation_iter_ids.shape[1] + answer_iter.shape[1])).to(input_ids.device)
            box_numbers_to_ans = torch.cat((box_numbers, no_object_pad_to_ans.unsqueeze(0)), dim = 1)
            ...
            for step in range(self.answer_max_len + 1):
                ...
                boxes_to_ans = torch.cat((boxes_to_ans, torch.zeros(1,8).unsqueeze(0).to(input_ids.device)), dim = 1)
                no_object = torch.LongTensor([self.special_tokens_ids[-1]]).to(input_ids.device)

                box_numbers_to_ans = torch.cat((box_numbers_to_ans, no_object.unsqueeze(0)), dim = 1)
