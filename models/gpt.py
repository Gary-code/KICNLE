...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # tokenizer_path = './pretrained_model/VQA-X/nle_gpt2_tokenizer_0'
        self.tokenizer = GPT2Tokenizer.from_pretrained(tokenizer_path)  # load tokenizer
        self.linear1 = nn.Linear(768, 768)
        self.linear2 = nn.Linear(768, 768)
        self.dense = nn.Linear(768, 768)
        self.dropout = nn.Dropout(0.3)
        self.layer_norm = nn.LayerNorm(768, 1e-5)


        # self.tokenizer.add_special_tokens({'additional_special_tokens': ['<knowledge>']})
        ...
        SPECIAL_TOKENS = ['<|endoftext|>', '<pad>', '<object>', '<question>', '<answer>', '<explanation>']
        ...
        # seq_len
        self.explanation_max_len = 30
        self.answer_max_len = 3
        self.question_max_len = 20
        self.knowledge_max_len = 8
        ...
        oscar_config = BertConfig.from_pretrained(oscar_caption_model_path, num_labels=2, finetuning_task='image_captioning')  # num_labels is 2 for classification and 1 for regression.
        oscar_tokenizer = BertTokenizerFast.from_pretrained(oscar_caption_model_path, do_lower_case=True)

        oscar_config.img_feature_dim = 2054
        oscar_config.img_feature_type = 'frcnn'
        oscar_config.hidden_dropout_prob = 0.3
        oscar_config.loss_type = 'sfmx'
        oscar_config.tie_weights = False
        oscar_config.freeze_embedding = False
        oscar_config.label_smoothing = 0
        oscar_config.drop_worst_ratio = 0
        oscar_config.drop_worst_after = 0
        oscar_config.output_attentions = True
        self.oscar = BertImgModel.from_pretrained(oscar_caption_model_path, from_tf=False, config=oscar_config)
        self.oscar.resize_token_embeddings(len(oscar_tokenizer))
        self.bert_intermediate = BertIntermediate(oscar_config)
        self.bert_output = BertOutput(oscar_config)
    ...
    def forward(
        ...
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        oscar_output, oscar_cls, _ = self.oscar(oscar_input_ids, img_feats=object_feat, attention_mask=oscar_mask, position_ids=None, token_type_ids=oscar_segment_ids, head_mask=None)

        
        encoder_attention_mask = torch.cat((encoder_attention_mask, oscar_mask), dim=1)
        encoder_hidden_states = torch.cat((encoder_hidden_states, oscar_output), dim=1)

        # only oscar
        # encoder_attention_mask = oscar_mask
        # encoder_hidden_states = oscar_output
